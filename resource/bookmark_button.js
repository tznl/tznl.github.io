...
extend(B.BookmarkButton.prototype, {
    setup: function () {
        ...
        this.buttonSource = this.link.getAttribute('data-hatena-bookmark-source');
        ...
        if (layout === 'simple') {
            ...
            if (this.buttonSource)
                buttonURL += '&source=' + encodeURIComponent(this.buttonSource);
    ...
    gotoTouchEntry: function () {
        if (this.defaultComment || this.buttonSource) {
          var addURL = B.origin + '/entry?url=' + encodeURIComponent(this.url);
          if (this.defaultComment) addURL += '&comment=' + encodeURIComponent(this.defaultComment);
          if (this.buttonSource) addURL += '&source=' + encodeURIComponent(this.buttonSource);
    ...
    clickHandler: function (event) {
        ...
        switch (this.mode) {
        case 'popup':
            B.BookmarkPanel.openPopup(this.url, this.getTitle(), this.defaultComment, this.buttonSource);
            break;
        case 'goto-touch':
            this.gotoTouchEntry();
            break;
...
B.BookmarkPanel = function (url, title, defaultComment, buttonSource) {
    this.url = url;
    this.title = title || '';
...
extend(B.BookmarkPanel, {
    currentPanel: null,
    makePanelURL: function (url, title, defaultComment, buttonSource) {
        ...
        if (buttonSource)
            panelURL += '&source=' + encodeURIComponent(buttonSource);
        return panelURL;
    },
    openPopup: function (url, title, defaultComment, buttonSource) {
        var panelURL = this.makePanelURL(url, title, defaultComment, buttonSource);
...
extend(B.BookmarkPanel.prototype, {
    setup: function () {
        ...
        var panelURL = B.BookmarkPanel.makePanelURL(this.url, this.title, this.defaultComment, this.buttonSource);
